typeof navigator === "object" && (function (global, factory) {
  ...
  function fetch$1(url, responseType = 'text', withCredentials = false) {
  ...
  const captions = {
    ...
    toggle(input, passive = true) {
      ...
      const active = is$1.nullOrUndefined(input) ? !toggled : input;
  ...
  const defaults$1 = {
    ...
    // 'fast-forward',
    // 'duration',
    'mute', 'volume', 'current-time', 'progress', 'captions', 'settings', 'pip', 'airplay',
    // 'download',
    'fullscreen'],
  ...
  const controls = {
    ...
    bindMenuItemShortcuts(menuItem, type) {
      ...
      on.call(this, menuItem, 'keydown keyup', event => {
        ...
        const isRadioButton = matches$1(menuItem, '[role="menuitemradio"]');
    ...
    createMenuItem({
      ...
      Object.defineProperty(menuItem, 'checked', {
        ...
        set(check) {
          // Ensure exclusivity
          if (check) {
            Array.from(menuItem.parentNode.children).filter(node => matches$1(node, '[role="menuitemradio"]')).forEach(node => node.setAttribute('aria-checked', 'false'));
          }
          menuItem.setAttribute('aria-checked', check ? 'true' : 'false');
        }
      });
      this.listeners.bind(menuItem, 'click keyup', event => {
        if (is$1.keyboardEvent(event) && event.key !== ' ') {
          return;
        }
        event.preventDefault();
        event.stopPropagation();
    ...
    updateProgress(event) {
      ...
      const setProgress = (target, input) => {
        ...
        const progress = is$1.element(target) ? target : this.elements.display.buffer;
    ...
    updateRangeFill(target) {
      ...
      const range = is$1.event(target) ? target.target : target;
    ...
    timeUpdate(event) {
      ...
      const invert = !is$1.element(this.elements.display.duration) && this.config.invertTime;
    ...
    durationUpdate() {
      ...
      const hasDuration = is$1.element(this.elements.display.duration);
    ...
    toggleMenu(input) {
      ...
      if (is$1.boolean(input)) {
        show = input;
      } else if (is$1.keyboardEvent(input) && input.key === 'Escape') {
        show = false;
  ...
  class Listeners {
    ...
    handleKey(event) {
      ...
      if (pressed) {
        ...
        if (is$1.element(focused)) {
          ...
          if (event.key === ' ' && matches$1(focused, 'button, [role^="menuitem"]')) {
            return;
          }
        }
    ...
    controls = () => {
      ...
      this.bind(elements.inputs.seek, 'mousedown mouseup keydown keyup touchstart touchend', event => {
        ...
        if (is$1.keyboardEvent(event) && !['ArrowLeft', 'ArrowRight'].includes(event.key)) {
          return;
        }
  ...
  const dashjs = {
    setup() {
      if (!is$1.function(window.dashjs)) {
        loadScript(this.config.urls.dashjs.sdk).then(() => {
  ...
  const hlsjs = {
    setup() {
      if (!is$1.function(window.Hls)) {
        loadScript(this.config.urls.hlsjs.sdk).then(() => {
  ...
  const vimeo = {
    ...
    ready() {
      ...
      if (!config.customControls) {
        fetch$1(format(player.config.urls.vimeo.api, src)).then(response => {
      ...
      Object.defineProperty(player.media, 'muted', {
        ...
        set(input) {
          const toggle = is$1.boolean(input) ? input : false;
      ...
      Object.defineProperty(player.media, 'loop', {
        ...
        set(input) {
          const toggle = is$1.boolean(input) ? input : player.config.loop.active;
        ...
        events: {
          ...
          onReady(event) {
            ...
            Object.defineProperty(player.media, 'muted', {
              ...
              set(input) {
                const toggle = is$1.boolean(input) ? input : muted;
                muted = toggle;
    ...
    ready() {
      ...
      if (player.config.sharing) {
        try {
          ...
          fetch(`${this.states.api_embed}/link`, param).then(res => res.json()).then(data => {
  ...
  class PreviewThumbnails {
    ...
    getThumbnails = () => {
      return new Promise(resolve => {
        ...
        // VTT urls
          ...
          const urls = is$1.string(src) ? [src] : src;
    ...
    getThumbnail = url => {
      return new Promise(resolve => {
        fetch$1(url, undefined, this.player.config.previewThumbnails.withCredentials).then(response => {
          const thumbnail = {
  ...
  class Plyr {
    ...
    set currentTime(input) {
      ...
      // Validate input
      const inputIsValid = is$1.number(input) && input > 0;
    ...
    get duration() {
      ...
      const duration = !is$1.number(realDuration) || realDuration === Infinity ? 0 : realDuration;
    ...
    set ratio(input) {
      ...
      if (!is$1.string(input) || !validateAspectRatio(input)) {
     ...
     */
    set autoplay(input) {
      this.config.autoplay = is$1.boolean(input) ? input : this.config.autoplay;
    }
    ...
    set pip(input) {
      ...
      const toggle = is$1.boolean(input) ? input : !this.pip;
